              ...
              <div class="col-md-3">
                <div class="form-group">
                  ...
                  <select class="form-control select2bs4" required="" name="user_id">
                    <option value="semua" <?= @$_POST['user_id'] == 'semua' ? 'selected' : '' ?>>Semua</option>
                    <?php
                    ...
                    foreach ($user as $ctr) : ?>
                      <?php if ($ctr['user_id'] != 0 && $ctr['user_level'] !== "kurir") { ?>
                        <option value="<?= $ctr['user_id'] ?>" <?= @$_POST['user_id'] == $ctr['user_id'] ? 'selected' : '' ?>><?= $ctr['user_nama'] ?></option>
                      <?php } ?>
                    <?php endforeach; ?>
                  </select>
                </div>
                ...
                <table id="laporan-transaksi-kasir" class="table table-bordered table-striped table-laporan">
                  ...
                  <?php
                    ...
                    $qq = "SELECT
                            ...
                            WHERE
                              invoice_date BETWEEN '" . $tanggal_awal . "' AND '" . $tanggal_akhir . "' 
                              AND invoice_cabang = $sessionCabang $whereuser
                            ORDER BY invoice_id DESC;";

                    $queryInvoice = $conn->query($qq);
                    if ($queryInvoice->num_rows > 0) {
                      while ($rowProduct = $queryInvoice->fetch_assoc()) {
