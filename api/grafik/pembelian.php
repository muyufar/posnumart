...
$q = "SELECT 
    ...
    i.invoice_pembelian_cabang,
    SUM(i.invoice_total) AS total_bayar,
    t.toko_nama
FROM 
    invoice_pembelian i
LEFT JOIN 
    toko t
ON
    i.invoice_pembelian_cabang = t.toko_cabang
WHERE 
    MONTH(i.invoice_date) = '$month'
    AND YEAR(i.invoice_date) = '$year'
GROUP BY 
    DATE(i.invoice_date), i.invoice_pembelian_cabang, t.toko_nama;";
...
foreach ($get as $row) {
  $invoiceCabang = $row['invoice_pembelian_cabang'];
  ...
  if ($index !== false) {
    $datasets[$tokoNama]['data'][$index] = (int)$row['total_bayar'];
  }
}
