...
try {
    ...
    if ($pakaiPergantianShift && shift_laporan_parse_jam($jamMulaiReq) === null && $jamMulaiReq !== '') {
        shift_laporan_json_exit(['ok' => false, 'message' => 'Format jam mulai tidak valid (gunakan HH:MM).']);
    }
    if ($pakaiPergantianShift && shift_laporan_parse_jam($jamSelesaiReq) === null && $jamSelesaiReq !== '') {
        shift_laporan_json_exit(['ok' => false, 'message' => 'Format jam selesai tidak valid (gunakan HH:MM).']);
    }
    ...
    error_log('laporan-pergantian-shift-data: ' . $e->getMessage());
    shift_laporan_json_exit([
        'ok' => false,
        'message' => $e->getMessage() ?: 'Terjadi kesalahan saat memuat laporan.',
    ]);
}
