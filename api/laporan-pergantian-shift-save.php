...
if (!isset($_SESSION['user_id'])) {
    shift_laporan_json_exit(['ok' => false, 'message' => 'Sesi habis, silakan login ulang.'], 401);
}

if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
    shift_laporan_json_exit(['ok' => false, 'message' => 'Metode tidak diizinkan.'], 405);
}
...
try {
    ...
    if (!preg_match('/^\d{4}-\d{2}-\d{2}$/', $tanggal)) {
        shift_laporan_json_exit(['ok' => false, 'message' => 'Format tanggal tidak valid.']);
    }
    ...
    if ($rowExist) {
        ...
        $oldSetoranTotal = 0;
        $qOldSetor = shift_laporan_db_query($conn, "
            SELECT COALESCE(SUM(setoran_kas), 0) AS total_setoran
            FROM shift_laporan_kasir
            WHERE shift_laporan_id = $laporanId
        ");
        $rowOldSetor = mysqli_fetch_assoc($qOldSetor);
        if ($rowOldSetor) {
            $oldSetoranTotal = (int) ($rowOldSetor['total_setoran'] ?? 0);
        }
        $sqlUpdate = "
    ...
    if (!isset($oldSetoranTotal)) {
        $oldSetoranTotal = 0;
    }
    $deltaSetoran = $newSetoranTotal - $oldSetoranTotal;
    if ($deltaSetoran > 0) {
        akun_posting_setoran_shift_ke_bank_bri($conn, $sessionCabang, $deltaSetoran);
    } elseif ($deltaSetoran < 0) {
        akun_posting_batal_setoran_shift_dari_bank_bri($conn, $sessionCabang, abs($deltaSetoran));
    }

    shift_laporan_db_commit($conn);

    shift_laporan_json_exit([
        'ok' => true,
    ...
    error_log('laporan-pergantian-shift-save: ' . $e->getMessage());
    shift_laporan_json_exit([
        'ok' => false,
