...
if (!$result || $result->num_rows === 0) {
    ...
    <script>
        ...
        window.location.href = '../satuan.php'; // Ganti dengan URL yang diinginkan
    </script>";
    exit;
}
...
// Header untuk Excel
$header = ['No', 'Nama Satuan', 'Status', 'Cabang'];
...
while ($row = $result->fetch_assoc()) {
        ...
        $sheet->setCellValue('B' . $rowNumber, $row['satuan_nama']);
        $sheet->setCellValue('C' . $rowNumber, $row['satuan_status']);
        $sheet->setCellValue('D' . $rowNumber, $row['satuan_cabang']);
