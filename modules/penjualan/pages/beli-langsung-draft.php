...
if( isset($_POST["inputbarcodeDraft"]) ){
  // var_dump($_POST);
  $draftBack = 'beli-langsung-draft?customer=' . urlencode((string) ($_GET['customer'] ?? '')) . '&invoice=' . urlencode((string) ($_GET['invoice'] ?? ''));

  // cek apakah data berhasil di tambahkan atau tidak
  if( tambahKeranjangBarcodeDraft($_POST) > 0 ) {
    echo "
      <script>
        document.location.href = " . json_encode($draftBack) . ";
      </script>
    ";
  }  
}
